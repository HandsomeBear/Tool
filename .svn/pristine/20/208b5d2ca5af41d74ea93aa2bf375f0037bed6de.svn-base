...
//-----------------------------------------------
@HSchedualObserver(executeType.PER_MINUTE)
public class PER_MINUTE extends HSchedualObserverer{

	public HSchedual.executeType executeType=HSchedual.executeType.PER_MINUTE;
	...
	public void doObserve(HSchedualTask task){
		try {
			...
			long unit=60*Integer.parseInt(units);
			if(unit<60) {return;}
